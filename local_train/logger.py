...
class BaseLogger(ABC):
    def __init__(self):
        ...
        self._performance_logs = defaultdict(list)
    ...
    # TODO: log_oracle never used? Note indeed that `compute_prd_from_embedding` does not exist anywhere.
    # @abstractmethod
    # def log_oracle(self,
    #                oracle,
    #                y_sampler,
    ...
    def log_performance(self, y_sampler, current_psi, n_samples):
        self._performance_logs['time'].append(time.time() - self._time)
        self._time = time.time()
        self._performance_logs['n_samples'].append(n_samples)
        self._performance_logs['func'].append(y_sampler.func(current_psi, num_repetitions=100000).detach().cpu().numpy())
        self._performance_logs['psi'].append(current_psi.detach().cpu().numpy())
        if not type(y_sampler).__name__ in ["BOCKModel", "BostonNNTuning", "RosenbrockModelDegenerate", "GaussianMixtureHumpModelDeepDegenerate"]:
            self._performance_logs['psi_grad'].append(y_sampler.grad(current_psi, num_repetitions=10000).detach().cpu().numpy())
        else:
            self._performance_logs['psi_grad'].append(np.zeros_like(current_psi.detach().cpu().numpy()))
...
class SimpleLogger(BaseLogger):
    ...
    def log_metric(self, name, value, step):
        # TODO: Epoch and step within epoch?
    ...
    def log_optimizer(self, optimizer):
        ...
        #
        # figure.legend()
        # self.optimizer_figs.append(figure)


    # TODO: log_oracle never used?
    ...
    def log_grads(self, oracle, y_sampler, current_psi, num_repetitions, n_samples=20, batch_size=None,
        ...
        self.log_metric('Mean grad norm', torch.norm(torch.mean(model_grad_value, dim=0, keepdim=True),
                                                                 dim=1).item(), step=self._epoch)
        self.log_metric('Mean grad var', torch.norm(torch.var(model_grad_value, dim=0, keepdim=True),
                                                                dim=1).item(), step=self._epoch)
        ...
        if log_grad_diff:
            ...
            # print(model_grad_value.shape, true_grad_value.shape)
            self.log_metric(
                'Mean grad diff', torch.norm(model_grad_value - true_grad_value).item(), step=self._epoch
            )


class SimpleGANLogger(SimpleLogger):
    """
    GANLogger that does not use the CometML Experiments class.
    """
    def __init__(self):
        super(SimpleGANLogger, self).__init__()
        self.metric_calc = Metrics((-10, 10), 100)

    def log_losses(self, losses):
        self.log_metric("d_loss", np.mean(losses[0]), step=self._epoch)
        self.log_metric("g_loss", np.mean(losses[1]), step=self._epoch)
    ...
    def log_validation_metrics(self, y_sampler, data, init_conditions, gan_model, psi_range, n_psi_samples=100,
        ...
        if calculate_validation_set and (self._epoch + 0) % 5 == 0:
            ...
            self.log_metric("average_mu_JS", np.mean(js), step=self._epoch)
            self.log_metric("average_mu_KS", np.mean(ks), step=self._epoch)
        ...
        self.log_metric("train_data_JS", train_data_js, step=self._epoch)
        self.log_metric("train_data_KS", train_data_ks, step=self._epoch)
        for order in range(1, 4):
            ...
            self.log_metric("train_data_diff_order_" + str(order), metric_diff, step=self._epoch)
            self.log_metric("train_data_order_" + str(order), moment_of_true, step=self._epoch)
...
class CometLogger(SimpleLogger):
    ...
    def log_performance(self, y_sampler, current_psi, n_samples, upload_pickle=True):
        super().log_performance(y_sampler=y_sampler, current_psi=current_psi, n_samples=n_samples)
        self._experiment.log_metric('Time spend', self._performance_logs['time'][-1], step=self._epoch)
        ...
        self._experiment.log_metric('Func value', self._performance_logs['func'][-1], step=self._epoch)
        self._experiment.log_metric('Used samples', self._performance_logs['n_samples'][-1], step=self._epoch)
        self._experiment.log_metric('Used samples cumm', np.sum(self._performance_logs['n_samples']), step=self._epoch)
        psis = self._performance_logs['psi'][-1]
        ...
        psi_grad = self._performance_logs['psi_grad'][-1]
